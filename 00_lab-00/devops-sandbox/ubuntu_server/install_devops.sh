...
VSCODE=OFF # supported value [ON, OFF]
GO=1.17 # supported value [ON, OFF, X.X]
AWSCLI=ON # supported value [ON, OFF]
BOTO3=ON # supported value [ON, OFF]
FIREFOX=OFF # supported value [ON, OFF]
TERRAFORM=1.9.4 # supported value [ON, OFF, X.X.X]
TERRAGRUNT=v0.82.4 # supported value [ON, OFF, vX.X.X]
DOCKER=ON # supported value [ON, OFF, X.X]
KEYPAIR=ON # supported value [ON, OFF]
ANSIBLE=2.10.0 # supported value [ON, OFF, X.X.X]
PACKER=1.10.0 # supported value [ON, OFF]
...
case $DOCKER in
  ON)
    ...
    # Ajouter le dépôt Docker pour focal
    echo \
    ...
    # Installer uniquement les paquets stables (éviter docker-model-plugin)
...
case $PACKER in
  ...
  *)
    #echo "Only ON and OFF value supported"
    echo "Installing specific Packer version: $PACKER"
    apt-get install -y unzip
    FILENAME="packer_${PACKER}_linux_amd64.zip"
    URL="https://releases.hashicorp.com/packer/${PACKER}/${FILENAME}"
    wget "$URL"
    unzip "$FILENAME"
...
echo "## Create /home/vagrant/devops-sandbox folder ##"
mkdir -p /home/vagrant/devops-sandbox
chown -R vagrant:vagrant /home/vagrant/devops-sandbox
